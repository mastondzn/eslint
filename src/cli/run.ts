...
export async function run(options: RuleOptions = {}) {
  ...
  package_.devDependencies['@antfu/eslint-config'] = `^${version}`;
  ...
  const antfuConfig = `${eslintIgnores.length > 0 ? `ignores: ${JSON.stringify(eslintIgnores)}` : ''}`;
  eslintConfigContent = package_.type === 'module'
    ? `
